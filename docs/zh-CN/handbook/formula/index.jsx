...
import { Input, Table, Typography, Card } from "antd";
...
const FormulaApp = () => {
    ...
    // 执行Formula计算
    const executeFormula = (inputFormula) => {
        try {
            const functionName = inputFormula.split("(")[0].trim();
            const argsString = inputFormula
                .substring(inputFormula.indexOf("(") + 1, inputFormula.lastIndexOf(")"))
                .trim();
            ...
            const args = argsString.split(",").map((arg) => {
                try {
                    ...
                    return arg.trim(); // 如果无法解析，保留原始字符串
                }
            });

            if (formulajs[functionName]) {
                const computedResult = formulajs[functionName](...args);
                // 检查返回值是否为 Date 对象，转换为字符串
                if (computedResult instanceof Date) {
                    setResult(computedResult.toISOString());
                } else {
    ...
    // 表格
    const renderFunctionTable = (functions) => {
        const columns = [
            {
                title: "Function",
                dataIndex: "title",
            ...
            { title: "Expected Result", dataIndex: "result", key: "result" },
            { title: "Function Definition", dataIndex: "definition", key: "definition" },
            { title: "Parameter Definition", dataIndex: "parameterDefinitions", key: "parameterDefinitions" },
        ];

        return <Table dataSource={functions} columns={columns} pagination={false} rowKey="title" />;
    };
    ...
    return (
        <div style={{ width: '100%' }}>
            ...
            <div style={{ zIndex: 1000, width: '100%' }}>
                <Card style={{ boxShadow: "0 4px 8px rgba(0, 0, 0, 0.1)", borderRadius: 8, marginTop: -15 }}>
                    <div style={{ display: "flex", alignItems: "center" }}>
                        <span style={{ fontWeight: "bold", marginRight: 10 }}>Formula:</span>
                        <Input
                            value={formula}
            ...
            <div style={{ marginTop: '20px' }}>
                <Typography>
                    <Paragraph>点击函数的 <strong>Example Call</strong> 将示例加载到上方的输入框中并执行。</Paragraph>
                </Typography>
                ...
                {functionsData.map((category) => (
                    <div key={category.category} style={{ marginBottom: "30px" }}>
                        ...
                        {renderFunctionTable(category.functions)}
                    </div>
                ))}
            </div>
        </div>
